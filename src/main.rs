...
fn midi_pitch(midi: u8, ref_key: f32, ref_pitch: f32) -> f32 {
    (midi as f32 - ref_key) * 12.0 / 31.0 + ref_pitch
}
...
fn main() {
    ...
    let ref_key: f32 = matches.value_of("ref_key").and_then(|k| k.parse().ok()).unwrap_or(60.0);
    let ref_pitch: f32 = matches.value_of("ref_pitch").and_then(|k| k.parse().ok()).unwrap_or(60.0);
            ...
            match msg.status {
                0x90 => {
                    let msg = MioscMessage::NoteOn(
                        msg.data1 as _,
                        midi_pitch(msg.data1, ref_key, ref_pitch),
